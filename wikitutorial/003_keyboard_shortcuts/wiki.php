...
function handle_post() {
  ...
  if( in_array($word,$protected_wiki_pages) ) {
    ...
    file_put_contents("pages/$word.txt",trim($new_src));
    header("Location: ${_SERVER['REQUEST_URI']}", true, 303);
    exit();
  } else {
...
function action_view() {
  ...
  $tmp = preg_replace_callback(WIKIWORD_REGEX,'wiki_word_to_link', $page_src)." ".edit_link();
...
function action_edit() {
  ...
  $output = str_replace(
    array("WIKI_WORD","WIKI_PAGE_SOURCE"),
    array($word,$page_src),
    $edit_template);
  echo $output;
}
